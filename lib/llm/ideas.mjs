...
- HOUSING: Regional HPI, listings, median prices with trend direction for CA/FL/Vegas. Cross-reference with Lumber for construction plays.
- US_INDEXES / GLOBAL_INDEXES: Live prices with daily change%. Look for regional divergence (US vs Europe vs Asia) as HEDGE ideas.
- SOFTS_AG: Coffee, Cocoa, Sugar, Corn, Wheat, Lumber futures. For inflation, weather, supply-chain, and housing-construction theses.
- ECONOMIC: Rates (yields, spreads, fed funds), inflation (CPI, PCE, core), labor (unemployment, payrolls), money supply, semiconductor production.
- ENERGY / METALS: Oil benchmarks, gold/silver. Geopolitical and inflation hedges.
- URGENT_OSINT / THERMAL / AIR_ACTIVITY / NUCLEAR_ANOMALY: Geopolitical and conflict signals. Use to inform energy, defense, and safe-haven ideas.
...
function compactSweepForLLM(data, delta, previousIdeas) {
  ...
  // Economic indicators (rates, inflation, labor, credit)
  if (data.fred?.length) {
    const key = data.fred.filter(f => ['VIXCLS', 'DFF', 'DGS10', 'DGS2', 'T10Y2Y', 'BAMLH0A0HYM2', 'DTWEXBGS', 'MORTGAGE30US', 'CPIAUCSL', 'CPILFESL', 'PCEPI', 'UNRATE', 'PAYEMS', 'M2SL', 'IPN2131N', 'TCU'].includes(f.id));
    sections.push(`ECONOMIC: ${key.map(f => `${f.id}=${f.value}`).join(', ')}`);
  }

  // Housing (regional)
  if (data.housing) {
    const fmt = (s) => {
      const h = data.housing[s];
      if (!h) return '';
      const t = (d) => d ? (d.direction === 'up' ? '+' : d.direction === 'down' ? '-' : '~') + (d.changePct != null ? `${d.changePct.toFixed(1)}%` : '') : '';
      return `${s.toUpperCase()}: HPI=${h.hpi}(${t(h.hpiTrend)}), Listings=${h.activeListings}(${t(h.listingsTrend)}), MedPrice=$${h.medianPrice}(${t(h.priceTrend)})`;
    };
    sections.push(`HOUSING: ${['california', 'florida', 'vegas'].map(fmt).filter(Boolean).join(' | ')}`);
  }
  ...
  // Markets (US + global indexes)
  if (data.markets) {
    const usIdx = (data.markets.indexes || []).map(q => `${q.name}=${q.price} (${q.changePct >= 0 ? '+' : ''}${q.changePct}%)`);
    const glIdx = (data.markets.global || []).map(q => `${q.name}=${q.price} (${q.changePct >= 0 ? '+' : ''}${q.changePct}%)`);
    const softs = (data.markets.softs || []).map(q => `${q.name}=${q.price} (${q.changePct >= 0 ? '+' : ''}${q.changePct}%)`);
    if (usIdx.length) sections.push(`US_INDEXES: ${usIdx.join(', ')}`);
    if (glIdx.length) sections.push(`GLOBAL_INDEXES: ${glIdx.join(', ')}`);
    if (softs.length) sections.push(`SOFTS_AG: ${softs.join(', ')}`);
  }
